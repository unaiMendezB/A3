...
for setting in settings:
    # Initialize the SOM
    som = MiniSom(x=setting['x'], y=setting['y'], input_len=X.shape[1], sigma=setting['sigma'], learning_rate=setting['learning_rate'], neighborhood_function=setting['neighborhood_function'])
    ...
    for x, target in zip(X, y):
        w = som.winner(x)
        plt.text(w[0], w[1], str(target), color=plt.cm.rainbow(target / len(set(y))),
